...
public class GenomicRangeQuery {
    ...
    public static int[] solution(String string, int[] start, int[] end) {

        String replace = string.replace('A', '1').replace('C', '2')
                .replace('G', '3').replace('T', '4');

        char[] dnaNumber = replace.toCharArray();

        Integer[] tab1=new Integer[string.length()];
        Integer[] tab2=new Integer[string.length()];
        Integer[] tab3=new Integer[string.length()];
        int count=1;
        for (int i = 0; i < dnaNumber.length; i++) {
            if (dnaNumber[i]=='1') count++;
            tab1[i]= count;
        }
        count=1;
        for (int i = 0; i < dnaNumber.length; i++) {
            if (dnaNumber[i]=='3') count++;
            tab3[i]= count;
        }
        count=1;
        for (int i = 0; i < dnaNumber.length; i++) {
            if (dnaNumber[i]=='2') count++;
            tab2[i]= count;
        }
        System.out.println(dnaNumber);
        ...
        for (int i = 0; i < result.length; i++) {
            if(!tab1[start[i]].equals(tab1[end[i]]) || dnaNumber[start[i]]=='1')  {
                result[i]=1;

                continue;
            }
            if(!tab2[start[i]].equals(tab2[end[i]]) || dnaNumber[start[i]]=='2')  {
                result[i]=2;
                continue;
            }
            if(!tab3[start[i]].equals(tab3[end[i]]) || dnaNumber[start[i]]=='3')  {
                result[i]=3;
                continue;
            }
            result[i]=4;
