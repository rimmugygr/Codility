...
System.out.println(solution(X,A));
    ...
    public static int solution(int xxx, int[] tabTime) {
        boolean[] isLeaves=new boolean[xxx];

        int sumToAcrros=0;
        for (int i = 1; i <= xxx; i++)   sumToAcrros+=i;

        int sumByFrog=0;
        for (int i = 0; i < tabTime.length; i++) {
            if (!isLeaves[tabTime[i]-1]) {//upadek liscia na puste miejsce
                isLeaves[tabTime[i]-1]=true;
                sumByFrog+=tabTime[i];
                if (sumByFrog==sumToAcrros) return i;
            }
        }
